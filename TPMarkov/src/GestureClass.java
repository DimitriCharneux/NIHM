...
public class GestureClass {
	...
	public boolean distanceValide(double dist) {
		System.out.println(dist);
		for (Template tem : examples) {
			Point2D a = tem.getPoints().get(0).getPoint();
			Point2D b = tem.getPoints().get(tem.getPoints().size() - 1).getPoint();
			double distab = distance(a, b);
			if (dist * 0.5 < distab && dist * 2 > distab)
				return true;
		}
