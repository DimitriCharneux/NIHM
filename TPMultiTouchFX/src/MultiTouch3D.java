...
public class MultiTouch3D extends Application{
	...
	public void start(Stage stage) {
		...
		scene.setOnMousePressed(e -> {
            ...
            if (bqueue.getPoint(1) == p)
            	ancienP1 = bqueue.getPoint(1);
		...
		scene.setOnMouseDragged(e -> {
            ...
            if (bqueue.getPoint(1) == p && firstPointIsInObject) {
				...
				Point2D diff = new Point2D(p.getX() - ancienP1.getX(), p.getY() - ancienP1.getY());
				
				double a = Math.atan(diff.magnitude() / (box.getWidth() / 2));
				
				Quaternion q2 = new Quaternion(new Point3D(-diff.getY(), diff.getX(), 0),-a);
